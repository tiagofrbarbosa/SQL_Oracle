SELECT P.CD_MULTI_EMPRESA,Count(*)
      FROM PACIENTE P
WHERE P.CD_PACIENTE IN(
SELECT CD_PACIENTE
       FROM
       (
SELECT Max(PACIENTE.CD_PACIENTE)             CD_PACIENTE
...
WHERE EXISTS (SELECT 'X'
                ...
                GROUP BY PACIENTE.CD_PACIENTE,PACIENTE.DS_CHECAPAC,PACIENTE.NM_PACIENTE,PACIENTE.DT_NASCIMENTO,PACIENTE.NM_MAE,PACIENTE.NM_USUARIO,SAME.CD_CAIXA,SAME.NR_MATRICULA_SAME,SAME.CD_CAD_SAME,PACIENTE.CD_MULTI_EMPRESA
                  --AND PACIENTE.NM_PACIENTE >= UPPER(:var || '%') AND PACIENTE.NM_PACIENTE <= UPPER(:var2 || '%')
 ORDER BY nm_paciente, DS_CHECAPAC
 )
 )
 GROUP BY P.CD_MULTI_EMPRESA
